...
def get_program_name(propid):
    program_time_fn = os.path.join(Path(__file__).parent.parent.parent.absolute(), "data/program_times.csv")
...
def merge_json_same_night(ras, decs, exptimes):
    ...
    for i in range(1, len(ras)):
        delra = (ras[i] - ras[i - 1]) * np.cos((decs[i] + decs[i - 1]) / 2.0)
        deg_separation = np.sqrt((delra) ** 2 + (decs[i] - decs[i - 1]) ** 2).to(u.deg).value
        total_dur += 220.0 / 100.0 * deg_separation + 29.0
        total_dur += exptimes[1]

    return np.round(avg_ra, 1), np.round(avg_dec, 1), np.round(total_dur / 60.0, 1)
...
def get_all_jsons_from_directory(directory, date):
    ...
    """
    base_jsons = glob.glob(os.path.join(directory, "*", date, "*.json"))
    prios = []
    ...
    low_prio_jsons = glob.glob(os.path.join(directory, "*", date, "*", "*.json"))
